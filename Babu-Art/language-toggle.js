(function () {
  const translations = {
    ...
    '形成表达 → 回到公众': 'Make an expression → return to the public',
    ...
    '第一站': 'FIRST SITE',
    '来自海内外学校的初高中生与成人志愿者走进东泉村。在工匠指导下，他们拆除屋面、整理旧瓦、制作土坯。真实工地上的第一课，是尽可能保存材料、做法与使用留下的历史信息。': 'Secondary students and adult volunteers from schools in China and abroad entered Dongquan Village. Guided by craftspeople, they opened the roof, sorted old tiles, and made adobe bricks. The first lesson on a real site was to preserve as much material, technique, and evidence of use as possible.',
    ...
    '含水量、填料均匀度、夯实力度——': 'Moisture, even filling, and tamping force —',
    '一块简单的土坯，也有自己的分寸。': 'even a simple adobe brick has its own measure.',
    ...
    '土坯来自当地最普通的泥土。从和泥、装模、夯实到脱模晾晒，学生亲手完成一块材料的诞生，也延续与原建筑相适应的传统做法。': 'The adobe bricks use the most ordinary local earth. From mixing and molding to tamping, releasing, and drying, students make a material come into being while continuing a traditional method suited to the original building.',
    ...
    '评价修复，不只看前后对比。原有材料有没有被谨慎保留？传统工艺有没有被理解与传递？空间能否重新进入生活？': 'Evaluate restoration by more than a before-and-after image. Were original materials carefully kept? Was traditional craft understood and passed on? Can the space return to daily life?',
    '修复成果': 'Restoration result',
    '院落与日常': 'Courtyard and daily life',
    '保留的构造': 'Construction retained',
    '平遥古城 · 精细、克制与可复查': 'Pingyao Ancient City · Precise, restrained, reviewable',
    ...
    '第二站': 'SECOND SITE',
    '第二年的实践，把注意力带到更小的尺度。面对受损墙基，学生学习先诊断、再操作。不多拆一块，不用新材料掩盖旧证据，也不放过一道不合格的工序。': 'The second year moved attention to a smaller scale. Facing damaged wall foundations, students learned to diagnose before acting. Do not remove one brick too many, cover old evidence with new material, or let an unqualified step pass.',
    ...
    '拆到哪里为止？材料是否相容？砂浆是否填实？每一步都需要判断，粗糙和侥幸都可能意味着重新来过。': 'Where should removal stop? Are the materials compatible? Is the mortar full? Every step requires judgment; carelessness can mean starting over.',
    '观察病害': 'Observe damage',
    ...
    '照片、草图和文字被组织成明信片与视觉作品。选择关键证据，让图像与文字相互解释，学生开始向他人说明：自己为什么在意这个地方。': 'Photographs, sketches, and words became postcards and visual works. By choosing key evidence and letting image explain text, students began to tell others why this place matters to them.',
    ...
    '什么值得保留？谁在使用？改变会影响谁？街巷尺度、院落关系、建筑细节与居民的使用方式，都会影响保护判断。': 'What is worth keeping? Who uses it? Who will change affect? Lane scale, courtyard relationships, architectural details, and residents’ habits all shape a restoration judgment.',
    ...
    '项目学员来自上海、深圳等地的实验学校、国际化学校及海外学习背景，覆盖中小学至大学阶段。成长也体现在如何提出问题、与他人配合、面对错误并完成表达。': 'Participants came from experimental and international schools in Shanghai, Shenzhen, and overseas backgrounds, from primary school to university. Growth also appears in how they ask questions, collaborate, face mistakes, and complete an expression.',
    ...
    '实践为课程提供真实素材与工作方法，不替代学校课程，也不构成分数或升学结果承诺。具体要求、提交数量及学术诚信规范，以所在学校与考试局当年官方文件为准。': 'The practice provides real material and methods; it does not replace school courses or promise grades or admissions results. Follow the current requirements, submission counts, and academic-integrity rules of each school and exam board.',
    '04 / 修复之后，故事继续': '04 / After restoration, the story continues',
    '日常重新发生': 'Everyday life returns',
    '一杯公益咖啡，': 'A cup of community coffee',
    '让老房子': 'lets the old house',
  ...
  function renderLanguage(language) {
    ...
    while ((node = walker.nextNode())) {
      ...
      if (!key) continue;
      const value = language === 'en' ? translations[key] || key : key;
